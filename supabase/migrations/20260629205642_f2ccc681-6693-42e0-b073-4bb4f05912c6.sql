...
-- user_usage: tracks plan + monthly search counter per user
...
-- Atomic check + increment with monthly rollover
...
BEGIN
  ...
  IF _row.searches_used + _cost > _limit THEN
    RETURN json_build_object(
      'allowed', false,
      'plan', _row.plan,
      'used', _row.searches_used,
      'limit', _limit,
      'remaining', GREATEST(_limit - _row.searches_used, 0)
    );
  END IF;

  _new_used := _row.searches_used + _cost;
  UPDATE public.user_usage
     SET searches_used = _new_used, updated_at = now()
   WHERE user_id = _user_id;
...
BEGIN
  ...
  RETURN json_build_object(
    ...
    'used', _row.searches_used,
    'limit', _l,
    'remaining', GREATEST(_l - _row.searches_used, 0)
  );
END;
$$;
...
-- 24h Google Places result cache (server-only)
...
-- No authenticated policy: only service_role touches it via the server.
